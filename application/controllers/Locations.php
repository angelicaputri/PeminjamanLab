...
class Locations extends CI_Controller {
	...
	public function index($page="")
	{
		...
		if (!$this->ion_auth->logged_in()){
			...
			$this->load->view('inv_location/index2');
	...
	public function add()
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules('kalab', 'Kalab', 'trim|required|callback__kalab_check');
	...
	*/
	...
	public function _kalab_check($kalab)
	{
		$datas = $this->locations_model->kalab_check($kalab);
		$total = count($datas->result());
		if ($total == 0) {
			return TRUE;
		}
			...
			$this->form_validation->set_message(
				'_kalab_check', '{field} with the name "'.$kalab.'" already exists.'
			);
			return FALSE;
		}
	}
	...
	*/
	public function edit($id)
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules('detail', 'Detail', 'trim');
			$this->form_validation->set_rules('kalab', 'Kalab', 'trim|required');
